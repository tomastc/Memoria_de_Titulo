# -*- coding: utf-8 -*-
# MODELO SIMPLIFICADO DE LAGUNA COSTERA - CON ANCHO CONSTANTE (bc=50) PERO SIMULANDO SOLO EN LOS INTERVALOS DE TIEMPO CON DATOS DE ANCHO REAL
# Se copia la lógica de intervalos, formatos, gráficos y reportes del código 2 (ancho variable).
# El ancho de boca se mantiene constante (parametros['bc']), pero la simulación se restringe a los períodos donde el archivo de anchos real tiene datos.
# Esto permite una comparación más justa con el modelo de ancho variable.
# Se mantienen todas las demás características: Qr y Zm interpolados, Z_L calculado con batimetría real, ecuaciones LaTeX,
...
class ModeloLagunaCostera:
    def __init__(self, archivo_caudal, archivo_marea, archivo_batimetria, parametros, archivo_anchos_nc=None):
        ...
        Si se proporciona archivo_anchos_nc, se cargan sus tiempos para determinar los intervalos de simulación.
        """
        ...
        # Indicador de ancho constante (aunque usemos los intervalos del archivo de anchos)
        ...
        # Cargar datos de ancho REAL solo para obtener los tiempos (no se usan los valores)
        if archivo_anchos_nc:
            self.cargar_datos_ancho_netcdf(archivo_anchos_nc)
        else:
            print("   ADVERTENCIA: No se proporcionó archivo de anchos. La simulación será continua (único intervalo).")

    def verificar_coherencia_temporal(self, fecha_inicio_sim, duracion_dias):
        """
        Verifica que el rango de la simulación se solape con los datos de entrada (caudal, marea y, si existen, anchos).
        """
        ...
        if sim_end < caudal_start or sim_start > caudal_end:
            raise ValueError("El rango de la simulación no se solapa con el del caudal")
        ...
        if sim_end < marea_start or sim_start > marea_end:
            raise ValueError("El rango de la simulación no se solapa con el de la marea")
        
        # Si hay datos de ancho, verificar también
        if hasattr(self, 'tiempos_ancho_nc'):
            ancho_start = self.tiempos_ancho_nc[0]
            ancho_end = self.tiempos_ancho_nc[-1]
            print(f"    Ancho (tiempos): {pd.to_datetime(ancho_start).strftime('%Y-%m-%d %H:%M')} a {pd.to_datetime(ancho_end).strftime('%Y-%m-%d %H:%M')}")
            if sim_end < ancho_start or sim_start > ancho_end:
                raise ValueError("El rango de la simulación no se solapa con el de los datos de ancho (tiempos).")
    ...
    def cargar_datos_ancho_netcdf(self, archivo_anchos_nc):
        """
        Carga SOLO los tiempos del archivo NetCDF de anchos reales.
        No se usan los valores de ancho ni estado, solo los momentos con datos.
        """
        try:
            ...
            if 'time' not in ds_anchos:
                print("   Variable 'time' no encontrada en el archivo de anchos. No se usarán intervalos.")
                self.bc_variable = False
                return
            
            # Extraer tiempos (asumimos que están en segundos desde época)
            tiempos_ancho = ds_anchos.time.values
            self.tiempos_ancho_nc = tiempos_ancho.astype('datetime64[s]')
            
            # Opcional: también podemos guardar los valores originales por si se quieren graficar
            if 'ancho_desembocadura' in ds_anchos:
                self.anchos_nc = ds_anchos.ancho_desembocadura.values
            if 'estado_apertura' in ds_anchos:
                self.estados_nc = ds_anchos.estado_apertura.values
            
            print(f"   Datos de ancho (tiempos) cargados para determinar intervalos:")
            print(f"    • Registros: {len(tiempos_ancho)}")
            print(f"    • Rango temporal: {pd.to_datetime(self.tiempos_ancho_nc[0]).strftime('%Y-%m-%d %H:%M')} a {pd.to_datetime(self.tiempos_ancho_nc[-1]).strftime('%Y-%m-%d %H:%M')}")
            
            ds_anchos.close()
        except Exception as e:
            print(f"   ERROR cargando datos de ancho: {e}")
            print("   Se procederá con simulación continua.")
    ...
    def determinar_intervalos_simulacion(self, fecha_inicio, fecha_fin):
        """
        Determina los intervalos de tiempo donde hay datos de ancho (según el archivo cargado),
        SIN EXTENDER (exactamente desde el primer al último dato de cada grupo).
        Si no hay datos de ancho, retorna un único intervalo continuo.
        """
        if not hasattr(self, 'tiempos_ancho_nc') or len(self.tiempos_ancho_nc) == 0:
            print("  • Simulación continua en todo el período (no hay datos de ancho para intervalos).")
        ...
        if len(tiempos_dentro) == 0:
            print("  • No hay datos de ancho en el rango de simulación. Se usará simulación continua.")
            return [(fecha_inicio, fecha_fin)]
    ...
    def ecuaciones_gobernantes(self, t, y):
        ...
        try:
            ...
            # Como el canal siempre está abierto (estado=1), solo consideramos esa rama
    ...
    def simular(self, fecha_inicio, duracion_dias, dt_minutes):
        ...
        print(f"\n════════════════════════════════════════════════════════════════")
        print(f"INICIANDO SIMULACIÓN POR INTERVALOS (ANCHO CONSTANTE {self.bc} m)")
        print(f"════════════════════════════════════════════════════════════════")
        ...
        resultados = {
            ...
            'tiempo_simulacion_seg': tiempo_total_sim
        }
    ...
    def graficar_resultados(self, resultados, archivo_salida=None):
        """
        Genera gráficos completos de los resultados siguiendo el formato del código 2.
        Se mantiene el orden: Z_m+Z_L, bc, Qc, Ac, Qr, V_L, con sharex y gaps.
        """
        ...
        # Función auxiliar para insertar NaN en saltos
        ...
        # 2) Ancho de la boca (constante) y puntos reales si existen
        ...
        t_sim_gap_dt = [pd.to_datetime(t) for t in t_sim_gap]
        ax2.plot(t_sim_gap_dt, bc_sim_gap, 'y-', linewidth=1.5, label=f'Ancho constante = {self.bc} m')
        
        if hasattr(self, 'anchos_nc'):
        ...
        ax2.set_title('ANCHO DE LA DESEMBOCADURA', fontsize=14, fontweight='bold', pad=15)
        ...
        ax3.plot(t_sim_gap_dt, Qc_gap, 'g-', linewidth=1.5,
                 label=f'Qc (min={np.min(Qc):.2f}, max={np.max(Qc):.2f} m³/s)')
    ...
    def guardar_resumen_txt(self, resultados, directorio_resultados, nombre_archivo=None):
        """
        Guarda un archivo TXT con un resumen detallado de todos los resultados (formato código 2).
        """
        if nombre_archivo is None:
            nombre_archivo = f'resumen_resultados_simulacion_intervalos_{datetime.now().strftime("%Y%m%d_%H%M%S")}.txt'
        ...
        try:
            with open(ruta_completa, 'w', encoding='utf-8') as f:
                f.write("=" * 100 + "\n")
                f.write("RESUMEN DETALLADO DE RESULTADOS DE SIMULACIÓN - LAGUNA COSTERA (ANCHO CONSTANTE, SIMULACIÓN POR INTERVALOS)\n")
                f.write("=" * 100 + "\n\n")
                ...
                f.write(f"Total de pasos de simulación (solo intervalos): {len(resultados['tiempos'])}\n")
...
def main():
    """Función principal de ejecución - ANCHO CONSTANTE, SIMULACIÓN POR INTERVALOS (usando tiempos de ancho real)"""
    
    # =============================================================================
    # CONFIGURACIÓN COMPLETA (bc constante)
    # =============================================================================
    ...
    CONFIG_ARCHIVOS = {
        ...
        'DIRECTORIO_RESULTADOS': r'C:\Users\Tomas\Desktop\Escritorio\memoria\03 Codigos\03 Codigo_modelo_simplificado\Resultados_modelo_lagunar_constante_intervalos-FINAL_2024-07'
    }
    ...
    PARAMETROS_MODELO = {
        'g': 9.81,
        'bc': 50,       # Valor constante en todo el tiempo
        'hc': 0.79,
        'Lc': 102,
        'n': 0.022,
        'ZL0': 0.25,
    ...
    print("════════════════════════════════════════════════════════════════")
    print("MODELO DE LAGUNA COSTERA - ANCHO CONSTANTE CON INTERVALOS DESDE DATOS REALES")
    print("SIMULACIÓN SOLO EN INTERVALOS CON DATOS DE ANCHO")
    print("════════════════════════════════════════════════════════════════")
    ...
    print(f"• Archivo de anchos (para intervalos): {ARCHIVO_ANCHOS_NC}")
    print("════════════════════════════════════════════════════════════════")
    
    # Verificar existencia del archivo de anchos (opcional, pero si no existe se usará continuo)
    if not os.path.exists(ARCHIVO_ANCHOS_NC):
        print(f"   No se encuentra el archivo de anchos NetCDF: {ARCHIVO_ANCHOS_NC}")
        print("   Se usará simulación continua (único intervalo).")
    ...
    try:
        # Inicializar modelo (con archivo de anchos para intervalos)
        print("\n════════════════════════════════════════════════════════════════")
        ...
        print(f"• Pasos simulados (solo intervalos): {len(resultados['tiempos'])}")
        ...
        # Generar gráficos con sufijo _intervalos
        nombre_grafico = f'Resultados_modelo_lagunar_constante_intervalos_FINAL_2024-07.png'
        ...
        nombre_netcdf = f'resultados_simulacion_constante_intervalos_FINAL_2024-07.nc'
        ...
        try:
            ...
            ds.attrs = {
                'descripcion': 'Resultados de simulación de laguna costera con ancho CONSTANTE, simulando solo en intervalos con datos de ancho real',
                ...
                'modelo': 'Laguna Costera - Runge-Kutta 4to orden, ancho constante, simulación por intervalos',
                ...
                'simulacion_por_intervalos': 'Sí, usando tiempos del archivo de anchos'
            }
